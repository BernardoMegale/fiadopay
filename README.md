...
| --------------------------------------------- | ---------------------------------------- | ------------------------------------------------------------- |
| **@PaymentMethod(type = "...")**              | `type` = nome do método (ex.: CARD, PIX) | Marca uma classe como método de pagamento plugável.           |
| **@AntiFraud(name = "...", threshold = ...)** | `name` = regra, `threshold` = limite     | Define uma regra de antifraude descoberta por reflexão.       |
| **@WebhookSink(url = "...")**                 | `url` = destino do webhook               | Registra o consumidor de webhooks processados pela aplicação. |
...
| **Factory / Plugin Registry**             | PluginRegistry + PluginScanner              | Permite registrar e instanciar plugins dinamicamente sem mudar o core.                     |
| **Strategy Pattern**                      | Métodos de pagamento e regras de antifraude | Permite trocar implementações (CARD, PIX, HighAmount etc.) sem alterar a lógica principal. |
